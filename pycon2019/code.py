"""For a detailed guide on all the features of the Circuit Playground Express
(cpx) library:
...
# Set this as a float from 0 to 1 to change the brightness. The decimal
...
while True:
    ...
    if board_id == 0:
        ...
        for p in range(10):
            ...
            cpx.pixels[p] = tuple(
                [
                    int(c * (10 - (pixel_number + p) % 10) / 10.0)
                    for c in color
                ],
            )
            cpx.pixels.show()
